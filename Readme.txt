ALTER TABLE individuals ADD COLUMN ascription TEXT;
ALTER TABLE individuals ADD COLUMN death_day INTEGER REFERENCES hijri_dates(id) ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE individuals ADD COLUMN birth_day INTEGER REFERENCES hijri_dates(id) ON DELETE SET NULL ON UPDATE CASCADE;
CREATE TABLE hijri_dates (id INTEGER PRIMARY KEY, year INTEGER NOT NULL, month INTEGER, day INTEGER);
...
CREATE TABLE notes (id INTEGER PRIMARY KEY, individual_id INTEGER NOT NULL REFERENCES individuals(id) ON DELETE CASCADE ON UPDATE CASCADE, note TEXT NOT NULL, type INTEGER);
CREATE TABLE collection_links (id INTEGER PRIMARY KEY, page_id INTEGER NOT NULL REFERENCES suite_pages(id) ON DELETE CASCADE ON UPDATE CASCADE, suite_id INTEGER NOT NULL REFERENCES suites(id) ON DELETE CASCADE ON UPDATE CASCADE, type INTEGER, UNIQUE (page_id,suite_id) ON CONFLICT IGNORE );
...
CREATE TABLE typos (id INTEGER PRIMARY KEY, table_name TEXT NOT NULL, query TEXT NOT NULL, primary_key INTEGER NOT NULL, UNIQUE(table_name,query,primary_key) ON CONFLICT IGNORE);